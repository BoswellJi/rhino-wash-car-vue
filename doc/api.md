...
| 字段名      | 类型   | 必填 | 说明       | 示例值                 |
| ----------- | ------ | ---- | ---------- | ---------------------- |
| roleIds    | Long| 必填  | 角色id       |1，2，3 可单独或者批量删除 |
...
| ----------- | ------ | ---- | ---------- | ---------------------- |
|roleId    | int| 是   | 角色id     |  1   |
|roleName    | String| 是   | 角色名称(不可更改)     |  "普通角色"    |
|roleKey    | String| 是    | 角色权限字符串（不可更改）       |"common" |
|dataScope    | int| 是     | 数据范围（1：全部数据权限 2：自定数据权限 3：本部门数据权限 4：本部门及以下数据权限）|
|deptIds     | array| 否    | 部门信息（当数据范围选择2时，调取/system/role/deptTree/{roleId} 接口，获取部门信息 ）|[1,2] |
...
**接口名**：`/system/role/authUser/unallocatedList`
...
**接口名**：`/system/role/authUser/cancel`
...
**接口名**：`/system/role/authUser/selectAll`
...
| menuName    | string | 否   |菜单名称      | "管理系统"              |
| icon | string | 否  | 菜单图标     | ""                |
| menuType  | int | 否   | 菜单类型   |（M目录 C菜单 F按钮）           |
| orderNum  | int | 否  | 显示顺序  | 0）     |
| isFrame   | int | 否   |是否为外链 0是 1否   | 1|
| isCache   | int | 否   | 是否缓存  |0缓存 1不缓存 |
| visible   | int| 否    | 菜单状态  | 1显示 0隐藏 |
| status    | int| 否     | 菜单状态  |1正常 0停用 |
...
| 字段名      | 类型   | 必填 | 说明       | 示例值                 |
| ----------- | ------ | ---- | ---------- | ---------------------- |
| postCode  | string | 否 | 岗位编码  | "IT"            |
| postName  | string | 否  | 岗位名称  | “硬件工程师”     |
| status  | int | 否  | 岗位状态  | 状态（1正常 0停用）     |
...
|&emsp;&emsp;postId| int | 岗位id     | 1    |
|&emsp;&emsp;postCode| String | 岗位编码     |“ceo”   |
|&emsp;&emsp; postName | String |岗位名称     | 软件开发    |
|&emsp;&emsp;postSort| int | 排序     |1  |
|&emsp;&emsp;status|  int |状态     |状态（1正常 0停用）|
|&emsp;&emsp;flag| boolean|用户是否存在此岗位标识 默认不存在     |false|


## 查询岗位详细

**接口名**：`/system/post/{postId}`
...
| 字段名      | 类型   | 必填 | 说明       | 示例值                 |
| ----------- | ------ | ---- | ---------- | ---------------------- |
| postCode    | int | 是   |岗位编码     | ceo              |
| postName | string | 是  | 部门名称     | 软件开发                |
| postSort  | int | 否   | 排序   |1           |
| status   | int| 否    | 菜单状态  | （1正常 0停用）默认1 |
| remark   | string | 否    | 备注  | “”|
...
| 字段名      | 类型   | 必填 | 说明       | 示例值                 |
| ----------- | ------ | ---- | ---------- | ---------------------- |
| postIds    | Long[]| 必填  | 岗位id       |[1,2,3] 可批量删除|
